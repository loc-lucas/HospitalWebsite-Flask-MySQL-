...
def getPat():        
    ...
    session['tab'] = 'list'
    session['tab2'] = 'day'
    session['patientList'] = data if data else [[]]
    return redirect(url_for('rerenderDoctor'))
...
def rerenderDoctor():    
    return render_template('doctor.html', departList=session['departList'], patientList=session['patientList'],
    patAbDis=session['patAbDis'], patDis=session['patDis'], patOut=session['patOut'], medList=session['medList'],
    lessMed=session['lessMed'], testList=session['testList'], nurseList=session['nurseList'],patList=session['patList'],
    patID=session['PatientID'], film=session['FTest'], test=session['Test'], medicine=session['Medicine'], diagnosis=session['Diagnosis'], tabidx=session['tab'], 
    tabidx2=session['tab2'], dates=session['dates'], filmList=session['filmList'])
...
# add new examination
    dID = session['id']
    dates = request.form['dates']
    shift = 'A' if request.form['shift'] else 'B'
    pID = request.form['patID']
    nID = request.form['nurseID']
    ...
    tmp=dates.split('-')
    # print(tmp)
    dates = tmp[0] + tmp[1]+ tmp[2]
    shiftID = '0' + str(data[0]) + str(dates) + str(shift)
